...
const renderBookmark = function (bookmark) {
  ...
  if (bookmark.expanded) {
    ...
    <button type='button' id='collapse' alt="collapse bookmark">-</button>
    ...
    return `<div class="css-bookmark" data-item-id="${bookmark.id}"><h2>${bookmark.title}</h2> 
      <p>${bookmark.rating}</p> 
      <button type='button' id='expand' alt="expand bookmark">+</button></div>`;
  }
};
...
function renderNewBookmarkForm() {
  return `<form class="css-form" id="js-new-bookmark-form">
        ...
        <fieldset>
            ...
            <textarea name="desc" id="js-bookmark-description-entry" cols="20" placeholder="Please describe the site here" required></textarea>
            <div id="rating-area">
